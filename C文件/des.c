...
void deskey(unsigned char *key,short edf, unsigned int *kn);
void cookey(register unsigned int *raw1, unsigned int *dough);
//void cpkey(register unsigned int *into);
//void usekey(register unsigned int *from);
//void des(unsigned char *inblock,unsigned char *outblock);
void scrunch(register unsigned char *outof, register unsigned int *into);
void unscrun(register unsigned int *outof, register unsigned char *into);
void desfunc(register unsigned int *block,register unsigned int *keys);

/*****************  DES Function  *****************/
unsigned int OPENCOMM_DesExpandEncKey(
		unsigned char *pbDesKey,
		unsigned int  ulDesKeyLen,
		unsigned char *pbDesEncKey,
		unsigned int *ulDesEncKeyLen);

unsigned int OPENCOMM_DesExpandDecKey(
		unsigned char *pbDesKey,
		unsigned int  ulDesKeyLen,
		unsigned char *pbDesDecKey,
		unsigned int *ulDesDecKeyLen);

unsigned int OPENCOMM_DesEncRaw(
		unsigned char *pbDesEncKey,
		unsigned int  ulDesEncKeyLen,
		unsigned char *pbInData,
		unsigned int  ulInDataLen,
		unsigned char *pbOutData,
		unsigned int *ulOutDataLen);

unsigned int OPENCOMM_DesDecRaw(
		unsigned char *pbDesDecKey,
		unsigned int  ulDesDecKeyLen,
		unsigned char *pbInData,
		unsigned int  ulInDataLen,
...
void deskey(unsigned char *key,short edf, unsigned int *kn)
{
	...
	for ( j = 0; j < 56; j++ ) 
	{
		l = pc1[j];
		m = l & 07;
		pc1m[j] = (((unsigned int) key[l >> 3] & (unsigned int)bytebit[m] ) ? 1:0);
	}
	for ( i = 0;i < 16; i++)
	{
		if ( edf == DE1 )	m = (15 - i) << 1;
		...
		unsigned int *ulDesEncKeyLen)
{
	...
	cookey(kn, dough);
	*ulDesEncKeyLen = DES_KEYBYTES;  //32 int = 128 bytes
		...
		unsigned int *ulDesDecKeyLen)
{
	...
	cookey(kn, dough);
	*ulDesDecKeyLen = DES_KEYBYTES;  //32 int = 128 bytes
...
unsigned int OPENCOMM_DesEncRaw(
		...
		unsigned int *ulOutDataLen)
{
	unsigned int work[2], ek[DES_KEYintS];
	...
	scrunch(cp,work);  // 8 bytes -> 2 int
	memcpy(ek, pbDesEncKey, ulDesEncKeyLen);
	desfunc(work,ek);
	unscrun(work,cp); // 2 int -> 8 bytes
...
unsigned int OPENCOMM_DesDecRaw(
		...
		unsigned int *ulOutDataLen)
{
	unsigned int work[2], dk[DES_KEYintS];
	unsigned char cp[DES_BLOCKLEN];
		...
		int           *pOutDataLen)
{
	...
	retval = OPENCOMM_DesExpandEncKey(DesKeyBuf, KeyLen,
		DesEncKeyBuf, (unsigned int *)&EncKeyLen);
	if(retval != 0)
		return retval;
	...
	for(i = 0; i < loops; i++)
	{
		retval = OPENCOMM_DesEncRaw(DesEncKeyBuf, EncKeyLen, pInData + i*8,
			8, pOutData + i*8, (unsigned int *)pOutDataLen);
		if(retval != 0)
			return retval;
	}
		...
		int           *pOutDataLen)
{
	...
	retval = OPENCOMM_DesExpandDecKey(DesKeyBuf, KeyLen,
		DesDecKeyBuf, (unsigned int *)&DecKeyLen);
	if(retval != 0)
		return retval;
	...
	int           *pOutDataLen)
{
	...
	rv = myic_DESEncrypt((unsigned char *)pDesKey, 8,
		padDate, (int)padDateLen, pOutData, pOutDataLen);
	if (rv != 0)
	{
		if (padDate != NULL)
	...
	int           *pOutDataLen)
{
	...
	//解密
	rv = myic_DESDecrypt((unsigned char *)pDesKey, 8,
		pInData, nInDataLen, tmpPlain, pOutDataLen);
	if (rv != 0)
	{
		if (tmpPlain != NULL) free(tmpPlain);
